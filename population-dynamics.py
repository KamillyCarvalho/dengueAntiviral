...
def graph_3D_generator(time,data1,data2,nome2,nome):
    fig = plt.figure()
    # Adiciona um subplot 3D
    ax = fig.add_subplot(111, projection='3d')

    x = time
    y = data1
    X, Y = np.meshgrid(x, y)
    Z = data2 # Exemplo de dados

    # Plota a superfície
    ax.plot_surface(X, Y, Z.reshape(X.shape), cmap='coolwarm')
    # Adiciona rótulos aos eixos
    ax.set_xlabel('Tempo')
    ax.set_ylabel('Antiviral')
    ax.set_zlabel(nome2)
...
for w in range(len(remediocount)):
    ...
    for n in range(time - 1):
        for r in range(kutta):
            ...
            dv[w,n,r] = remediocount[w] * k * i[w,n,r] - gamma * v[w,n,r] - a[2] * s[w,n,r] * v[w,n,r]
